...
def train(data_path, model_path, model, batch_size, lr, from_checkpoint=False, min_batch_size=BATCH_SIZE):
    ...
    history = model.fit(train_gen, steps_per_epoch=STEPS_PER_EPOCH*(min_batch_size // batch_size), validation_steps=VALIDATION_STEPS, epochs=EPOCHS, 
...
def loop_setup_transformer():
  batch_sizes = [4, 2] # [25,50,75,100]
  min_batch_s = min(batch_sizes) # [0, 50, 100, 150]
  learning_rates = [0.005, 0.001, 0.0005, 0.0001]#[0.05,0.01,0.005,0.001]
  dropouts = [0, 0.4] # [0, 0.4]
  num_transformer_blocks = [8, 4, 2]
  mlp_units = [[64], [64, 64], [32], [32, 32]]
        ...
        for dropout in dropouts:
          for num_transformer_block in num_transformer_blocks:
            for mlp_unit in mlp_units:
              model_path = 'models\\' + f"transformer_batch_size{batch_size}_new\\{STEPS_PER_EPOCH}_{EPOCHS}_{SEQ_LEN_PAST}_{SEQ_LEN_FUTURE}_{dropout}_{batch_size}_{learning_rate}_{num_transformer_block}_{mlp_unit}\\"
              try:
                if not os.path.exists(model_path):
                  os.makedirs(model_path)
                model = setup_model_transformer(dropout, num_transformer_block, mlp_unit)
                mode = 'train'
                train(data_path, model_path, model, batch_size, learning_rate, min_batch_size=min_batch_s)
              except:
                print("ERROR: ", model_path)
                # TODO: delete model path
                if os.path.exists(model_path):
                  os.removedirs(model_path)

    # mlp_unit = [128,128,128]
    # model_path = 'models\\' + f"transformer_batch_size{batch_size}\\{STEPS_PER_EPOCH}_{EPOCHS}_{SEQ_LEN_PAST}_{SEQ_LEN_FUTURE}_{0}_{4}_{learning_rates[3]}_{2}_{mlp_unit}\\"
    # if not os.path.exists(model_path):
    #   os.makedirs(model_path)
...
def finetune_1_transformer_setup():
    with tf.device(GPU_STRING):
       
      # models\transformer_batch_size1\100_50_840_300_0_4_0.0001_2_[128, 128, 128]
      data_path = DATA_PATH_PROCESSED
      steps_per_epoch = 100
      epochs = 50
      seq_len_past = 840
      ...
      model_path = 'models\\' + f"transformer_finetuning\\{steps_per_epoch}_{epochs}_{seq_len_past}_{seq_len_future}_{dropout}_{batch_size}_{learning_rate}_{num_transformer_blocks}_{mlp_units}\\"
      
      if not os.path.exists(model_path):
          os.makedirs(model_path)

      model = tf.keras.models.load_model(model_path + 'model-058-mae5.1008.h5')

      model = setup_model_transformer(dropout, num_transformer_blocks, mlp_units)

      mode = 'train'

      train(data_path, model_path, model, batch_size, learning_rate, min_batch_size=batch_size)

def finetune_2_transformer_setup():
    with tf.device(GPU_STRING):
      ...
      model_file_path = model_path + '\\model-025-mae1.0893.h5'
      if os.path.isfile(model_file_path):
      # model = tf.keras.models.load_model(model_path + '\\model-025-mae1.0893.h5')
        print("model EXISTS!")
...
def finetune_3_transformer_setup():
    with tf.device(GPU_STRING):
      ...
      # model_path = 'models\\' + MODEL_NAME + '\\'
      model_path = 'models\\' + f"transformer_experimenting\\{steps_per_epoch}_{epochs}_{seq_len_past}_{seq_len_future}_{dropout}_{batch_size}_{learning_rate}_{num_transformer_blocks}_{mlp_units}"
...
def run():
    ...
    # finetune_2_transformer_setup()
    finetune_3_transformer_setup()
    # lopp_setup_mlp()
    # loop_setup_conv()
    # loop_setup_lstm()
    # normal_setup()
